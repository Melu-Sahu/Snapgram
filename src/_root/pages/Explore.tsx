...
const Explore = () => {
  ...
  const { data: searchedPosts, isFetching: isSearchFetching } = useSerarhPostMutation(debouncedSearch);
      ...
      <div className="flex flex-wrap gap-9 w-full max-w-5xl">
        {shouldShowSearchResults ? (
          ...
          posts.pages.map((item, index) => (
            <GridPostList key={`page-${index}`} posts={item.documents} showUser={true} showState={true} />
          ))
        )}
      </div>
